...
MKF <- function(y, types, models, ind_mod, crit_eff = 0.2) {
  ...
  # resampling step if effective sample size falls below crit * (sample size)
  crit <- crit_eff

  # only keep unique type draws and weigh resulting draws equally 
  # (alternative: keep track of duplicates and reweigh corresponding draws -> computationally intensive for large M)
  ...
  for (t in 1:TT) {
    ...
    # (unless 1 path has close to prob 1 -> continue with that path only)

    wt_vec <- exp(ln_wt_vec)
    crit_single <- 1 - 1e-05 # critical value to keep only single type draw

    if ((Mt_eff / Mt < crit || max(wt_vec) > crit_single) && Mt > 1) {

      # if posterior weight of a path is close to 1 only keep that path
      if (max(wt_vec) > crit_single) {

        ind_max <- which(wt_vec == max(wt_vec))
        Mt <- 1

        # only keep type draw "ind"
        ...
        aux <- sample(1:Mt, size = Mt, replace = TRUE, prob = wt_vec)
