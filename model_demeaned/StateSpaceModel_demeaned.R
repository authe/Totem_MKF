SP_model_demeaned <- function(rho, omega, sig_u, sig_e, sig_n, ord, S, W, sig_z=0, tol=1e-15){
  ...
  # (ii) raw submission data is demean with cross-sectional mean
  # weak submitters: s_{i,t} = a_t + theta_t^(1) + x_{i,t},
  # strong submitters: s_{i,t} = a_t + theta_t^(0) + z_{i,t},
  # cross-section mean m_t = a_t + (1-omega) theta_t^(0) + omega theta_t^(1)
  # consensus price (lagged) p_t = m_{t-1} + sig_e e_t
  # demeaned data:
  # strong: y_{i,t} = s_{i,t} - m_t = omega (theta_t^(0) - theta_t^(1)) + z_{i,t},
  # weak: y_{i,t} = s_{i,t} - m_t = x_{i,t} - (1-omega) (theta_t^(0) - theta_t^(1)),
  ...
  # demeaned consensus price q_t = sig.e e_t
  ...
  # "strong" submitters i=W+1,...,S submit omega (theta_t^(0) - theta_t^(1)) + sig.z z_{i,t}
  if (W < S){
    Zt[(W + 2):(S + 1), 1] <- omega
    Zt[(W + 2):(S + 1), 1] <- -omega
  }
